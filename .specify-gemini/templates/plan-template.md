...
1. **思考先行**: 是否已在研究中明确所有假设? 是否对不明确的需求进行了澄清? 
2. **简洁至上**: 该方案是否为解决问题的最小可行方案? 是否包含任何超前或单次使用的抽象? 
3. **手术刀式变更**: 计划的变更范围是否严格限于本功能? 是否避免了对无关代码的重构? 
4. **目标驱动执行**: 是否为每个阶段和任务定义了清晰的、可衡量的验证成功标准? 
